...
export function Logo({ className, link = true }: { className?: string; link?: boolean }) {
  const mark = (
    <span className={cn('inline-flex items-center gap-1.5 sm:gap-2', className)}>
      <img src={carlinkLogo} alt="" className="h-9 sm:h-11 w-auto" aria-hidden />
      <span className="text-2xl sm:text-3xl tracking-tight leading-tight" style={{ fontFamily: "'Sora', sans-serif" }}>
